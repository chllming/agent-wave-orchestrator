...
When bumping `package.json` version, keep the shipped surface aligned across `README.md`, `CHANGELOG.md`, `docs/README.md`, `docs/plans/current-state.md`, `docs/plans/migration.md`, `docs/reference/coordination-and-closure.md`, `docs/reference/runtime-config/README.md`, `releases/manifest.json`, `.wave/install-state.json`, tracked `.wave/upgrade-history/`, and this file. The current versioned operating guide is `docs/guides/recommendations-0.8.9.md`; when the package version moves, update that guide name and every reference to it in the same change.
